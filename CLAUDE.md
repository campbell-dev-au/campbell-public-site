...
- **Case studies:** `lib/case-studies.ts` is the source of truth for the index (`/case-studies`) *and* for the sitemap — adding an entry there is what gets a case study listed and indexed; the detail page itself is a hand-written route under `app/case-studies/<slug>/page.tsx`, and the route must also be added to `e2e/routes.ts`. Two exist: `wordpress-integration-review`, the write-up of a real (anonymized) vibe-code health check engagement, linked from the health-check service; and `simple-gift-registry`, a write-up of Campbell's own product (live at simplegiftregistry.com.au, public repo) used as the proof point for vibe-to-production — its stats and claims are drawn from the actual `simple-gift-registry` repo, so re-check them there before editing.
