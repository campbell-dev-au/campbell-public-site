...
Campbell Davis's freelance marketing site — a static Next.js site advertising five services: website development, business process analysis/improvement, bespoke software development, vibe-coded prototype health checks, and vibe-to-production development. No backend, no CMS, no database: this is a handful of static pages plus one flagship case study.
...
- **Next.js App Router + TypeScript.** Every route is statically generated (SSG) — no server data fetching, no route handlers.
...
- **Contact is intentionally backend-free.** `/contact` is a `mailto:` link plus social links — no form, no API route, no email service integration. If a form gets added later, it should be a Route Handler backed by a transactional email API (e.g. Resend), not a client-only solution.
